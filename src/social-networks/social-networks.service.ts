...
export class SocialNetworksService {
  ...
  async create(data: CreateSocialNetworkDto, user?: any) {
    const exists = await this.repo.findOne({ where: { companyId: data.companyId } });

    if (exists) {
      throw new ConflictException('Essa empresa já possui redes sociais cadastradas');
    }
  ...
  async findByCompany(companyId: number) {
    const social = await this.repo.findOne({ where: { companyId } });

    if (!social) {
      throw new NotFoundException('Redes sociais não encontradas para essa empresa');
    }
  ...
  async update(companyId: number, data: UpdateSocialNetworkDto, user?: any) {
    ...
    if (data.facebook !== undefined && data.facebook !== before.facebook)
      fieldsChanged.push(`Facebook: "${before.facebook ?? '-'}" → "${data.facebook ?? '-'}"`);
    if (data.instagram !== undefined && data.instagram !== before.instagram)
      fieldsChanged.push(`Instagram: "${before.instagram ?? '-'}" → "${data.instagram ?? '-'}"`);
    if (data.linkedin !== undefined && data.linkedin !== before.linkedin)
      fieldsChanged.push(`LinkedIn: "${before.linkedin ?? '-'}" → "${data.linkedin ?? '-'}"`);
    if (data.other !== undefined && data.other !== before.other)
      fieldsChanged.push(`Outras: "${before.other ?? '-'}" → "${data.other ?? '-'}"`);
